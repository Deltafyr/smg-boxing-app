...
import { 
  ...
  Calendar, Info as InfoIcon, Target, DownloadCloud, ClipboardCheck,
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-t-[2.5rem] p-8 pb-12 shadow-2xl animate-in slide-in-from-bottom-10 overflow-y-auto max-h-[90vh]">
        <div className="flex justify-between items-center mb-6 sticky top-0 bg-slate-900 z-10 py-2">
          ...
          <button onClick={onClose} className="p-2 bg-slate-800 rounded-full text-slate-400 transition-colors hover:bg-slate-700"><X size={20}/></button>
        </div>
        {children}
      </div>
    </div>
  ...
  return (
    <div className="p-6 h-full flex flex-col items-center justify-center animate-in slide-in-from-bottom-4 min-h-[80vh]">
      ...
      <div className={`w-72 h-72 rounded-full border-[15px] transition-all duration-700 flex flex-col items-center justify-center bg-slate-900/60 shadow-2xl relative ${isResting ? 'border-cyan-500 shadow-cyan-500/20' : 'border-slate-800 shadow-rose-500/20'}`}>
        ...
        <div className="text-7xl font-black text-white font-mono tracking-tighter">{formatTime(timeLeft)}</div>
        <div className={`text-xs font-black mt-3 uppercase tracking-widest ${isResting ? 'text-cyan-400' : 'text-rose-500'}`}>{isResting ? 'REPOS' : `Round ${round}`}</div>
      </div>
      <div className="flex gap-4 w-full max-w-xs mt-12">
        ...
        <button onClick={() => { setIsActive(false); setTimeLeft(workTime); setRound(1); setIsResting(false); }} className="p-5 border border-slate-800 rounded-3xl text-slate-400 hover:text-white transition-all"><RotateCcw/></button>
      </div>
    </div>
  );
};
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const unsubTournaments = onSnapshot(collection(db, ...path, 'competitions'), s => {
    ...
    return () => { unsubProfile(); unsubChat(); unsubMembers(); unsubFights(); unsubAgenda(); unsubTournaments(); };
  }, [user, selectedCompId]);
            ...
            <div className="pb-32">
                ...
                <div className="p-5 space-y-6 animate-in fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <FuturisticCard borderColor="gold" onClick={() => setView('tournament')} className="flex flex-col items-center gap-4 py-8 col-span-2 group active:scale-95 transition-all">
                      ...
                      <span className="text-[10px] font-black text-white uppercase tracking-widest">Arène Elite & FFKMDA Data</span>
                    </FuturisticCard>
                  </div>
              ...
              {view === 'tournament' && (
                  <TournamentModule 
                    ...
                    onNavigate={() => setView('home')} onScan={scanFFKMDA} isSyncing={isSubmitting}
                  />
              )}
              ...
              {view === 'chat' && (
                <div className="flex flex-col h-screen pb-20 bg-slate-950 animate-in slide-in-from-right-4">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar pr-2">{messages.map((m: any) => (<div key={m.id} className={`flex ${m.uid === user.uid ? 'justify-end' : 'justify-start'}`}><div className={`p-3 rounded-2xl text-sm max-w-[85%] ${m.uid === user.uid ? 'bg-cyan-600 text-white rounded-tr-none shadow-lg shadow-cyan-900/20' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-slate-700'}`}>{m.uid !== user.uid && <div className="text-[9px] text-cyan-500 mb-1 font-black uppercase tracking-tighter">{m.sender}</div>}<div className="leading-relaxed">{m.text}</div></div></div>))}<div ref={messagesEndRef} /></div>
...
// --- MODULE COMPETITION COMPLET (ADAPTÉ FIRESTORE) ---
      ...
      {activeTab === 'GESTION' && (
        <div className="space-y-6">
           ...
           <div className="space-y-4">
              <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Gestion des Matchs</h3>
              {members.filter((m: any) => currentCompetition?.participants?.includes(m.id)).map((m: any) => (
                <div key={m.id} className="space-y-2">
                   <div className="flex justify-between items-center border-b border-slate-800 pb-2 px-1">
                      <span className="text-sm font-black text-white italic uppercase tracking-tighter leading-none">{m.firstName} {m.lastName}</span>
                      <button onClick={async () => {
                         await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'fights'), {
                             competitionId: selectedCompId, fighterId: m.id, fighterName: `${m.firstName} ${m.lastName}`,
                             fightNumber: 1, ring: "1", helmetColor: "Bleu", status: "Pending"
                         });
                      }} className="bg-cyan-600 p-1.5 rounded-lg text-white shadow-lg active:scale-90 transition-all"><Plus size={16}/></button>
                   </div>
                   <div className="grid gap-2">
                      {fights.filter((f:any) => f.fighterId === m.id && f.competitionId === selectedCompId).map((f: any) => (
                        <FuturisticCard key={f.id} onClick={() => setSelectedFight(f)} borderColor={f.status === 'Finished' ? 'slate' : 'cyan'} className={f.status === 'Finished' ? 'opacity-40 grayscale' : 'hover:border-cyan-400'}>
                           <div className="flex justify-between items-center">
                              <div className="flex items-center gap-3">
