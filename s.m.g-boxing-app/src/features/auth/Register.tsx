...
const Register: React.FC<RegisterProps> = ({ onLogin, onNavigate }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Création du profil avec Catégorie LOISIR imposée par défaut.
      const newUser: User = {
        id: uid,
        ...
        category: 'Loisir', // <--- Blocage sur Loisir par défaut
      ...
      else setError('Erreur lors de la création du profil.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] p-4 space-y-4">
      <div className="w-full max-w-sm flex justify-start mb-2 pt-4">
        <button onClick={() => onNavigate('login')} className="text-slate-500 hover:text-cyan-400 flex items-center text-xs font-bold uppercase transition-colors"><ArrowLeft size={14} className="mr-1" /> Retour</button>
      </div>

      <div className="text-center w-full mb-4">
        <h1 className="text-2xl font-black text-white italic tracking-tighter uppercase mb-1">Dossier Combattant</h1>
        <p className="text-[10px] text-cyan-400 font-mono uppercase tracking-widest">Incorporation S.M.G</p>
      </div>
      ...
      <FuturisticCard className="w-full max-w-sm max-h-[70vh] overflow-y-auto custom-scrollbar pr-2 pb-10" borderColor="cyan">
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[9px] text-slate-500 font-bold uppercase mb-1 block">Type de Membre</label>
                <select name="memberType" value={formData.memberType} onChange={handleChange} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white text-xs outline-none focus:border-cyan-500">
                  <option value="Enfant">Enfant (-12)</option><option value="Junior">Junior (12-17)</option><option value="Adulte">Adulte (+18)</option><option value="Parent">Parent (Non-prat.)</option>
                </select>
              </div>
              <div>
                <label className="text-[9px] text-slate-500 font-bold uppercase mb-1 block">Poids (kg) - Opt.</label>
                <input name="weight" type="number" step="0.1" value={formData.weight} onChange={handleChange} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white text-xs outline-none focus:border-cyan-500" placeholder="Ex: 75.5" />
              </div>
            </div>
            {/* Note d'information pour la catégorie */}
            <p className="text-[8px] text-slate-500 italic mt-1 bg-slate-900/50 p-2 rounded border border-slate-800">Le statut d'incorporation par défaut est <strong className="text-cyan-500">LOISIR</strong>. Un entraîneur de la S.M.G pourra vous élever au rang de Compétiteur suite à une évaluation.</p>
          </div>
          ...
          <div className="pt-4 pb-2 border-t border-slate-800">
            <button type="submit" disabled={isLoading} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-black py-4 rounded-xl flex justify-center items-center uppercase text-xs shadow-lg shadow-cyan-600/20 active:scale-95 transition-all">
              {isLoading ? 'GÉNÉRATION...' : 'VALIDER L\'INCORPORATION'} <ChevronRight size={16} className="ml-2" />
            </button>
          </div>
        </form>
      </FuturisticCard>
