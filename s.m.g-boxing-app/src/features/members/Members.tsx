...
// Fonctions pures sorties du composant pour alléger le thread principal
...
const getAge = (dateString: string | undefined | null) => {
...
export default function Members({ currentUser }: { currentUser: User }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setPalmares(pList);
      } catch(e) { console.error("[CORTEX] Erreur chargement Membres:", e); }
      setIsLoading(false);
    };
    fetchData();
  }, []);
  ...
  const handleRoleChange = useCallback(async (userId: string, newRole: string) => {
    if (userId === currentUser.id) {
      alert("Interdit : Vous ne pouvez pas modifier votre propre niveau d'accréditation.");
      return;
    }
    try {
    ...
    } catch(e) { alert("Erreur de synchronisation du rôle."); }
  }, [currentUser.id, selectedMember]);

  // [SYNAS] Hyper-Optimisation : Le filtrage de recherche est mis en cache
  const filteredMembers = useMemo(() => {
    return members.filter(m => m.name.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [members, searchQuery]);

  // [SYNAS] Hyper-Optimisation : Calculs de profil mis en cache
  const profileData = useMemo(() => {
    ...
    const category = selectedMember.birthDate
      ? calculateCategoryFFKMDA(new Date(selectedMember.birthDate).getFullYear(), selectedMember.gender === 'Femme' ? 'F' : 'M')
      : 'N/C';

    return { memberPalmares, highestTitle, age, category };
  }, [selectedMember, palmares]);

  if (selectedMember && profileData) {
    const { memberPalmares, highestTitle, age, category } = profileData;
    ...
    return (
      <div style={{ height: '100vh', overflowY: 'auto', paddingBottom: '150px' }} className="w-full px-4 pt-4">
        <div className="max-w-lg mx-auto space-y-6">
          ...
          {isStaff && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-lg relative overflow-hidden animate-fade-in">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-lg relative overflow-hidden">
            ...
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-800 pb-2 mb-4 flex items-center">
              <Activity size={14} className="mr-2 text-cyan-500"/> Fiche Biométrique
            </h3>
