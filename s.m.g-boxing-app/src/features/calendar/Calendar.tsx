...
export default function CalendarPage({ currentUser }: { currentUser: User }) {
  ...
  const handleAddEvent = async (e: React.FormEvent) => {
    e.preventDefault(); if (!newTitle || !newDate) return; setIsLoading(true);
    try {
  ...
  return (
    <div className="p-4 pb-24 max-w-lg mx-auto">
      <div className="flex justify-between items-center mb-8"><div><h2 className="text-2xl font-black text-white italic tracking-tighter uppercase leading-none">Agenda Club</h2><p className="text-[10px] text-purple-400 font-mono tracking-widest uppercase mt-1">Planning S.M.G</p></div>{isStaff && (<button onClick={() => setShowAddForm(!showAddForm)} className="bg-purple-600/20 text-purple-400 p-2 rounded-xl border border-purple-500/50 hover:bg-purple-600/40 transition-colors"><Plus size={20} /></button>)}</div>
      {showAddForm && isStaff && (
        <FuturisticCard className="mb-6 animate-fade-in" borderColor="slate" title="Nouvel Événement">
          <form onSubmit={handleAddEvent} className="space-y-3">
            <input type="text" placeholder="Titre..." value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white text-xs outline-none focus:border-purple-500" required />
            <div className="grid grid-cols-2 gap-3">
              ...
              <select value={newType} onChange={e => setNewType(e.target.value as EventType)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white text-xs outline-none focus:border-purple-500"><option value="Événement Club">Événement Club</option><option value="Championnat">Championnat</option><option value="Coupe">Coupe</option></select>
            </div>
